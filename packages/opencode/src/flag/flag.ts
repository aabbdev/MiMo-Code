...
export const Flag = {
  ...
  // thing a session could be missing: the alternative for a payload past the
  // window is no answer at all. Its own risks are bounded rather than absent: a
  // kernel that outlives a call needs a way out (so `reset`, an idle sweep and the
  // session's death are the three), and a loaded kernel keeps its payload's memory
  // for the session's lifetime (+92 MB RSS measured for 4.5 MB of payload, not
  // returned by disposal). Set MIMOCODE_EXPERIMENTAL_REPL_TOOL=false to remove it
  // from the tool roster entirely.
  MIMOCODE_EXPERIMENTAL_REPL_TOOL: !falsy("MIMOCODE_EXPERIMENTAL_REPL_TOOL"),
